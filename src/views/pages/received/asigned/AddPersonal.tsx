...
const AddPersonal = ({ open, toggle, onSelect, tabs, currentIndex }: AddPersonalType) => {
    ...
    const hasAvailableUsers = shifts.some(shift =>
        shift.hrs.some(hr =>
            hr.services.some(svc => svc.users.some(u => !assignedUsers.has(u._id)))
        )
    )
                    ...
                    {!hasAvailableUsers ? (
                        ...
                        >
                            <Typography variant="h4">
                                Personal no disponible en este momento
                            </Typography>
                        </Box>
                    ) : (
                        shifts.map((shift, index) => (
                            <Grid item xs={12} key={shift._id || index}>
                                <Box sx={{ mt: 4 }}>
