...
const schema = yup.object().shape({
    ...
    marker: yup.object({
        ...
        .required('El campo marca de vehículo es requerido'),

    otherMarker: yup
        .string()
        .when('marker', {
            is: (val: MarkerType | null) => val?.name === 'Otro',
            then: schema => schema.required('Debe especificar otra marca'),
    ...
    type: yup.object({
        ...
        .required('El campo tipo de vehículo es requerido'),

    otherType: yup
        .string()
        .when('type', {
            is: (val: TypeType | null) => val?.name === 'Otro',
            then: schema => schema.required('Debe especificar otro tipo de vehículo'),
            otherwise: schema => schema.notRequired()
        }),
            ...
            <form autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
                <fieldset style={{ border: `1.5px solid ${theme.palette.primary.main}`, borderRadius: 10, paddingTop: 20 }}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={6}>
                            <FormControl fullWidth sx={{ mb: 6 }}>
                                <InputLabel id="marker-select">Marca de vehiculo</InputLabel>
                                <Controller
                                    name="marker"
                                    control={control}
                            ...
                            <Grid item xs={6}>
                                <FormControl fullWidth sx={{ mb: 3 }}>
                                    <Controller
                                        ...
                                        render={({ field: { value, onChange } }) => (
                                            <TextField
                                                label='Especifica otra marca de vehiculo'
                                                onChange={onChange}
                        ...
                        <Grid item xs={6}>
                            <FormControl fullWidth sx={{ mb: 6 }}>
                                <InputLabel id="type-select">Tipo de vehiculo</InputLabel>
                                <Controller
                                    name="type"
                                    control={control}
                                    render={({ field: { onChange, value } }) => (
                            ...
                            <Grid item xs={6}>
                                <FormControl fullWidth sx={{ mb: 3 }}>
                                    <Controller
                                        ...
                                        render={({ field: { value, onChange } }) => (
                                            <TextField
                                                label='Especifica otro tipo de vehiculo'
                                                onChange={onChange}
