...
import { dowUser, upUser } from 'src/store/user'
...
const Schedules = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: ({ row }: CellType) => {
                return (
                    <Typography noWrap variant='body2'>{row.post}</Typography>
                )
            }
        },
        {
    ...
    const hadleErrors = (error: any) => {
        ...
        if (error.response?.status === 401) {
            message = 'No tienes autorización para solicitar usuarios. solicite autorización al administador del sistema';
        }
        if (error.response?.status === 404) {
            message = "No podemos encontrar usuarios, error 404. Por favor contacte al desarrollador del sistema para más asistencia."
        }
        Swal.fire({
            title: '¡Error!',
            text: message,
    ...
    useEffect(() => {
        const initApi = async () => {
            try {
                const response = await api.get('/users', { filter: '', skip: page * pageSize, limit: pageSize })
