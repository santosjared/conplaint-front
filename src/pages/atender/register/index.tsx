...
const schema = yup.object().shape({
    ...
    otra_denuncia: yup
        ...
        .when('tipo_denuncia', {
            ...
            then: (schema) => schema.required('Debe especificar otro tipo de denuncia'),
    ...
    const onSubmit = async (data: DenunciaType) => {
        try {
            ...
            if (atendido?.status === 'warning' && data?._id) {
                delete newData._id
                await instance.put(`/confirmed/${data._id}`, newData)
            } else {
                await instance.post('confirmed', newData)
            }
            fetch()
        } catch (error) {
